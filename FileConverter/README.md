...
CREATE DATABASE fileconverter;
CREATE DATABASE fileconverter_hangfire;
\q
```
...
#### Windows:
1. Скачайте FFmpeg для Windows с официального сайта: https://ffmpeg.org/download.html (выберите "Windows builds")
2. Распакуйте архив
3. Создайте папку `FileConverter/ffmpeg` если её нет
4. Скопируйте файлы `ffmpeg.exe`, `ffplay.exe` и `ffprobe.exe` в эту папку

**Примечание**: Файлы ffmpeg специально исключены из репозитория из-за их большого размера (>140 МБ каждый).
